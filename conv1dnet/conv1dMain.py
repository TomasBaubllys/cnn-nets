...
from utils.nnutils import test, train, plot_hists, plot_acc, generate_confusion_matrix, export_predictions_to_csv
import argparse

# parses the arguments from the command line, used for testing variuos hyperparameters
def parse_arguments():
    ...
    parser.add_argument("-cm", "--confusion_matrix", action="store_true", default=False, help="Generates a confusion matrix for Conv1dNetSmall")
    parser.add_argument("-exp", "--export_predictions", action="store_true", default=False, help="Exports first 30 predictions of a Conv1dNetSmall to a csv file")
    return parser.parse_args()

if __name__ == "__main__":
    # create the standart datasets
    ...
    # create base models for base testing
    ...
    if args.base_test_train: # base test but with training
    ...
    if args.batch_norm_test: # batch norm test used on the smallest model
    ...
    if args.optimizer_test: # optimizer test used on the smallest model
    ...
    if args.learning_rate_test: # learning rate test used on the smallest model
    ...
    if args.confusion_matrix: # generates a confusion matrix
        model = Conv1dNetSmall()
        generate_confusion_matrix(model=model, dataset=dataset_test, weights_path="smalllr0_01_model_weights.pth", save_path="confusionsmall.jpg")
    if args.export_predictions: # generates a a classification table for random 30 values from the dataset
        model = Conv1dNetSmall()
        export_predictions_to_csv(model=model, dataset=dataset_test, weights_path="smalllr0_01_model_weights.pth", filename="small.csv")